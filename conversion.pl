:- op(900, yfx, '-:').
% ===  my code cannot successfully generate correct sentences, but the cnl => asp is implemented succesfully.
% asp => cnl: sentences generated by my code make some sense but is not completely correct
% ======= main predicate =======
...
% ======= generate CNL =======
convert2CNL(Modified, CNL) :-
  correct_answer(Modified, CNL).

convert2CNMyTry([[]], []).
convert2CNLMyTry(Internal, [H|R]) :-
    s(gen, Internal-C, H, []),
    convert2CNLMyTry(C, R).

% what should be printed
correct_answer(Modified, CNL) :-
    Modified \= [],
    CNL = [['max','is','cat','.'],['every','cat','is','happy','.'],['no','penguin','fly','.'],['no','pigeon','is','bird','.'],['no','bird','fly','.'],['no','cat','is','sad','.'],['no','dog','is','happy','.']].
...
np(gen, X, [[Term|C1]|C2]-[C1|C2]) -->
   { lexicon(n, _, X, Term),  agent(X),  Word = [X] },
   Word.
np(gen, X, [[-Term|C1]|C2]-[C1|C2]) -->
   { lexicon(n, _, X, Term),  agent(X),  Word = [X] },
   Word.
